...
class DataSetCreator:
  ...
  def process_and_plot_train_test_images(data_augumentation="random"): 
    ...
    for path in Path('/tmp/dataset/train/').rglob('*.jpg'):
      ...
      result = subprocess.run(["python", "/content/gdrive/MyDrive/mfr_test_repo/MaskTheFace/mask_the_face.py",'--path', path, '--mask_type', data_augumentation, '--verbose', '--write_original_image'],  stdout=subprocess.PIPE, text=True)   
      print(result.stdout)
    train_list = get_image_list_from_folder('/tmp/dataset/train/')
